...
export const getDataMoveiReview = createAsyncThunk(
  "catch Review",
  async (id, thunkApi) => {
...
export const getDataMoveiBackDrops = createAsyncThunk(
  "catch BackDrops",
  async (id, thunkApi) => {
    const { rejectWithValue } = thunkApi;
    try {
...
const userDataMoveiSocial = createSlice({
  name: "data social",
  initialState: data,
  extraReducers: (builder) => {
    // Review Movei
    builder.addCase(getDataMoveiReview.pending, (state) => {
      state.LoadingReview = true;
    });
    builder.addCase(getDataMoveiReview.fulfilled, (state, action) => {
      state.LoadingReview = false;
      state.userReview = action.payload;
    });
    builder.addCase(getDataMoveiReview.rejected, (state, action) => {
      state.LoadingReview = false;
      state.erorReview = action.payload.message;
    });
    // BackDrops Movei
    builder.addCase(getDataMoveiBackDrops.pending, (state) => {
      state.LoadindBackDrops = true;
    });
    builder.addCase(getDataMoveiBackDrops.fulfilled, (state, action) => {
    ...
    builder.addCase(getDataMoveiBackDrops.rejected, (state, action) => {
      state.LoadindBackDrops = false;
...
export const backDataSocialMovei = userDataMoveiSocial.reducer;
